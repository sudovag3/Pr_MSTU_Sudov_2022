...
def ChordMethod(a, b, evalx, eps):
    ...
    if (function(a, evalx) * ddy.subs(x, a)) > 0: # проверяем неподвижность точки a
        x0 = b
        while True:
            x0 = x0 - (function(x0, evalx) * (a - x0)) / (function(a, evalx) - function(x0, evalx)) # вычисляем начальное приближение по заданной формуле
        ...
        x0 = a
        while True:
            x0 = x0 - (function(x0, evalx) * (b - x0)) / (function(b, evalx) - function(x0, evalx)) # вычисляем начальное приближение по заданной формуле
            y = function(x0, evalx) # значение функции в данной точке
            if abs(y) < eps: # условие выхода из цикла |y(x0)| < e
                break
    end = timer()
...
def NewtonsMethod(a, b, evalx, eps):
    ...
    if (function(a, evalx) * ddy.subs(x, a)) > 0: # проверяем неподвижность точки a
        x0 = a
        while True:
            x0 = x0 - function(x0, evalx) / eval(str(dy.subs(x, x0))) # вычисляем начальное приближение по заданной формуле
            y = function(x0, evalx) # значение функции в данной точке
            if abs(y) < eps: # условие выхода из цикла |y(x0)| < e
                break
